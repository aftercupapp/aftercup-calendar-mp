...
function mergeEvents(fetched, stored) {
    ...
    const preAddedEvents = fetched.map(event => ({ ...event,
        preAdded: true
    }));
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        ...
        if (response.ok) {
            ...
            fetchedEvents = Array.isArray(data) ? data : (data.events || []);
        }
    } catch (error) {
        console.warn('Could not load external events (events.json). Using local data only.', error);
    }
...
function renderEvents() {
    ...
    sortedEvents.forEach(event => {
        ...
        // Logic to handle Pre-Added events:
        // 1. If it has a link, open it (and prevent edit).
        // 2. If it has NO link, do nothing (and prevent edit).
        if (event.preAdded === true && event.link) {
            eventItem.style.cursor = 'pointer';
            eventItem.onclick = (e) => {
                if (e.target.closest('a, .delete-btn, .task-checkbox-label, .read-more-btn')) {
                    return;
                }
            ...
            // Pre-added but no link: just display, no interaction.
...
function renderUpcomingEvents() {
    ...
    for (const dateStr in groupedEvents) {
        ...
        dateEl.textContent = eventDateObj.toLocaleDateString(undefined, {
            weekday: 'long',
            year: 'numeric',
            month: 'long',
            day: 'numeric'
...
function setPinkMode() {
    changeTheme('pink');
}

function setPastelGreenMode() {
    changeTheme('pastel-green');
}

function setCoffeeMode() {
    changeTheme('coffee');
}
...
function setPastelVioletMode() {
    changeTheme('pastel-violet');
}

function setCherrySodaMode() {
    changeTheme('cherry-soda');
}
...
let spCalendarGridEl, spMonthTitleEl, spCurrentDate = new Date(),
    spSelectedDayCell = null;
...
function initShiftPlannerElements() {
    ...
    document.getElementById("sp-prev-month").addEventListener("click", () => {
        spCurrentDate.setMonth(spCurrentDate.getMonth() - 1);
        spRenderCalendar();
    });
    document.getElementById("sp-next-month").addEventListener("click", () => {
        spCurrentDate.setMonth(spCurrentDate.getMonth() + 1);
        spRenderCalendar();
    });
}
...
function openShiftPlanner() {
    ...
    history.pushState({
        popup: 'shift-planner-popup-main'
    }, '', null);
    hideActionButtons();
